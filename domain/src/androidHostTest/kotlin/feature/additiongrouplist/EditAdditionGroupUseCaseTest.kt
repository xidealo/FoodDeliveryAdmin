package test.feature.additiongrouplist
...
class EditAdditionGroupUseCaseTest {
    ...
    @Test
    fun `invoke successfully updates addition group`() =
        runTest {
            // Given
            val token = "token"
            val uuid = "123"
            ...
            val updateAdditionGroup =
                updateAdditionGroupMock.copy(
                    name = "Updated Name",
                    isVisible = true,
                    singleChoice = false,
                    priority = 1,
    ...
    @Test
    fun `invoke throws NotFindAdditionGroupException when group not found`() =
        runTest {
            val token = "token"
            coEvery { dataStoreRepo.getToken() } returns token
    ...
    @Test
    fun `invoke does nothing when name and flags are unchanged`() =
        runTest {
            val token = "valid_token"
            val uuid = "123"
            val existingGroup =
    ...
    private val updateAdditionGroupMock =
        UpdateAdditionGroup(
            name = "",
            isVisible = false,
            singleChoice = false,
