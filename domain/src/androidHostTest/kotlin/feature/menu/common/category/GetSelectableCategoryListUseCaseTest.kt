package test.feature.menu.common.category
...
class GetSelectableCategoryListUseCaseTest {
    private val getCategoryListUseCase: GetCategoryListUseCase =
        mockk {
            coEvery { this@mockk.invoke(refreshing = false) } returns
                listOf(
                    getMockCategory(1),
                    getMockCategory(2),
                    getMockCategory(3),
    ...
    @Test
    fun `returns all not selected categories when input uuid list is empty`() =
        runTest {
            val expected =
                listOf(
                    SelectableCategory(
    ...
    @Test
    fun `returns selected categories when input uuid list is not empty`() =
        runTest {
            val expected =
                listOf(
                    SelectableCategory(
