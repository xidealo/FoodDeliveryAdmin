...
class GetSeparatedAdditionListUseCaseTest {
    ...
    @BeforeTest
    fun setup() {
        getSeparatedAdditionListUseCase = GetSeparatedAdditionListUseCase(
    ...
    @Test
    fun `return separated lists when additions exist`() = runTest {
        // Given
        val token = "token"
        ...
        coEvery { additionRepo.getAdditionList(token, refreshing = true) } returns listOf(
            visibleAddition1,
            visibleAddition2,
            hiddenAddition
        )
    ...
    @Test
    fun `sort additions inside grouped list by name`() = runTest {
        // Given
        val token = "token"
